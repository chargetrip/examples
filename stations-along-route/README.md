...
For this example, we use **Tesla model S**, **Amsterdam** as an origin, and **Utrecht** as a destination point.
...
4. We can use the `stationsAlongRoute` object tot display alternative stations.
5. We can also query route details for information like total distance, duration of a trip, consumption etc. You can see all available fields to query in the [Chargetrip API documentation](https://docs.chargetrip.com/#get-route-details).
6. The final step is to show the route on a map. We use [MapboxGL JS](https://docs.mapbox.com/mapbox-gl-js/overview/#quickstart) in this example.
